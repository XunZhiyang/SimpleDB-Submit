    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        switch (joinOp) {
            ...
            case GREATER_THAN_OR_EQ:
                int field1 = stats.get(table1Alias).id(field1PureName);
                int field2 = stats.get(table2Alias).id(field2PureName);
                if (field1 == -1 || field2 == -1) card = card1 * card2 / 2;
                else {
                    double pMin = stats.get(table1Alias).estimateSelectivity(field1, joinOp, new IntField(stats.get(table2Alias).min(field2)));
                    double pMax = stats.get(table2Alias).estimateSelectivity(field1, joinOp, new IntField(stats.get(table2Alias).max(field2)));
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        for (int i = 1; i <= joins.size(); i++) {
            for (Set<LogicalJoinNode> subset : enumerateSubsets(joins, i)) {
                ...
                cache.addPlan(subset, cost, c.card, c.plan);
            }
        }
