...
public class Aggregate extends Operator {
    ...
    public void open() throws NoSuchElementException, DbException,
        ...
        super.open();
        child.open();
        Type t = gfield == -1 ? null : child.getTupleDesc().getFieldType(gfield);
        switch (child.getTupleDesc().getFieldType(afield)) {
            case INT_TYPE:
                IntegerAggregator integerAggregator = new IntegerAggregator(gfield, t, afield, aop);
                while (child.hasNext()) integerAggregator.mergeTupleIntoGroup(child.next());
                iterator = integerAggregator.iterator();
                break;
            case STRING_TYPE: {
                StringAggregator stringAggregator = new StringAggregator(gfield, t, afield, aop);
